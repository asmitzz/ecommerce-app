...
const Sidebar = ({className,setShowSidebar}) => {
    const { name,isUserloggedIn,signout } = useAuth();
  ...
  return (
    <div>
         ...
         <section className="profile__pic">
          ...
          {  isUserloggedIn ? <p style={{margin:'0.5rem 0'}}>{name}</p> : <p style={{margin:'0.5rem 0'}}>Login/Signup</p> }
         </section>
         ...
         <section className="sidebar__links__section">
           ...
           { isUserloggedIn && <> 
           <Link className="sidebar__link" to="/orders" onClick={closeSidebar}><i className="fas fa-box-full"></i>&nbsp;&nbsp;Your Orders</Link>
